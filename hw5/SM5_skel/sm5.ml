  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
       ...
       *)
       let baseL = ref [] in
       let rec cSF : svalue -> unit = fun x -> (
         match x with
         | V a -> (
           cVF a
          )
         | P (_, com, env) -> (
           let _ = List.iter cCF com in
           let (_, eee) = List.split env in
           (List.iter cEF eee)
          )
         | M (a, b) -> (cEF b)
         | _ -> ()
        ) and
        cVF : value -> unit = fun x -> (
          match x with
          | L (a, b) -> (
            let _ = (baseL := !baseL @ [a]) in
            try (let aaaa = List.assoc (a,b) m in cVF aaaa) with Not_found -> ()
           )
          | R ll -> (
            let (all , bll) = (List.split ll) in
            let fff x = (
              let (a, b) = x in
              let _ = (baseL := !baseL @ [a]) in
              try (let aaaa = List.assoc (a,b) m in cVF aaaa) with Not_found -> ()
              (* if not ((try List.assoc (a,b) m with Not_found -> ()) = ()) then (cVF (List.assoc (a,b) m)) else () *)
            ) in
            List.iter fff bll
           )
          | _ -> ()
        ) and
        cEF: evalue -> unit = fun x -> (
          match x with
          | Loc (aaq,bbq) -> cSF (V (L (aaq, bbq)))
          | Proc (_, com, env) -> (
            let _ = List.iter cCF com in
            let (_, eee) = List.split env in
            (List.iter cEF eee)
        ...
        cKF : (command * environment) -> unit = fun x -> (
          let (cp, eeq) = x in
          let (_,eq) = List.split eeq in
          let _ = List.iter cCF cp in
          List.iter cEF eq
        ) in
        let (_, eeee) = List.split e in
        let _ = List.iter cSF s in
        let _ = List.iter cEF eeee in
        let _ = List.iter cCF c in
        let _ = List.iter cKF k in
        let concatF x = (
          let (q, _) = x in
          let (aaa, bbb) = q in
          if (List.mem aaa !baseL) then (let _ = reachable_locs := !reachable_locs @ [(aaa,bbb)] in ()) else (())
        ) in
        let _ = List.iter concatF m in
